...
from regex_test import cleanCondor, cleanCondorUser, cleanGlidein, findEmail, replaceIP,findUserIds
...
class TestFilter(unittest.TestCase):
    ...
    def test_condor(self):
        ...
        startd = "condor_logs/job.5.StartdLog.txt"
        ...
        # eu_file.close()
        # #print(sr_data)
        self.assertFalse(line.find(s_email) != -1)
        self.assertNotRegex(line, IP_REGEX[0])
        self.assertNotRegex(line, IP_REGEX[1])
        for x in range(0, len(s_ids)):
            self.assertFalse(line.find(s_ids[x]) != -1)
